...
public:
    ...
    virtual std::vector<char> compile(const std::string &source, const std::string &target, const std::string &entry, bool optimize)
    {
        ...
        ETargetVersion trnTarget = language == "glsles" ? ETargetGLSL_ES_100 : language == "glsles3" ? ETargetGLSL_ES_300
                                                                           : language == "glsl3"     ? ETargetGLSL_140
                                                                                                     : ETargetGLSL_110;
        glslopt_target optTarget = language == "glsles" ? kGlslTargetOpenGLES20 : language == "glsles3" ? kGlslTargetOpenGLES30
                                                                              : language == "metal"     ? kGlslTargetMetal
                                                                                                        : kGlslTargetOpenGL;
...
private:
    ...
    template <typename T>
    static std::vector<T> consumeData(HRESULT hr, ID3DBlob *buffer, ID3DBlob *messages)
    {
        if (SUCCEEDED(hr))
        {
            if (messages)
...
int compileOne(int argc, char **argv)
{
    ...
    ShaderCompilerHLSL2GLSL compiler(metal ? "metal" : glsles ? (gl3 ? "glsles3" : "glsles")
                                                              : (gl3 ? "glsl3" : "glsl"));
