...
namespace RBX
{
    ...
    static shared_ptr<FileMeshData> readMeshFromV2OrV3(const std::string& data, size_t offset, bool isV3)
    {
        shared_ptr<FileMeshData> mesh(new FileMeshData());
        ...
        {
            if (header.cbSize != (sizeof(FileMeshHeader) - sizeof(header.cbLODStride) - sizeof(header.num_LODOffsets)))
                throw std::runtime_error("Error reading mesh data: incompatible stride");
        ...
        for (int i = 0; i < header.num_vertices; i++)
        {
            ...
            if (header.cbVerticesStride != 40)
                mesh->vnts[i].r = mesh->vnts[i].g = mesh->vnts[i].b = mesh->vnts[i].a = 255;
        }
    ...
    static shared_ptr<FileMeshData> readMeshFromV4OrV5(const std::string& data, size_t offset, bool isV5)
    {
        shared_ptr<FileMeshData> mesh(new FileMeshData());
...
namespace RBX
{
    shared_ptr<FileMeshData> ReadFileMesh(const std::string& data)
    {
        ...
        if (data.compare(0, 12, "version 1.00") == 0)
            ...
            result = readMeshFromV2OrV3(data, versionEnd + 1, false);
        else if(data.compare(0, 12, "version 3.00") == 0 || data.compare(0, 12, "version 3.01") == 0)
			result = readMeshFromV2OrV3(data, versionEnd + 1, true);
