    ...
    , meshContentProvider(0)
{
    ...
    else
    {
        for (Text::Font font = Text::FONT_LEGACY; font != Text::FONT_LAST; font=Text::Font(font+1))
        {
            ...
            #if EMSCRIPTEN
                typesetters[font].reset(new TypesetterBitmap(getTextureManager(), std::string("/content/") + kFontPaths[font], kTexturePath, legacyHeightScale, device->getCaps().retina));
            #else
...
{
    #ifdef EMSCRIPTEN
    	shaderManager->loadShaders("/shaders", device->getShadingLanguage(), /* consoleOutput= */ true);
    #else
