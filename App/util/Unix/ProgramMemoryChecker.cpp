    ...
    , lastCompletedTime(Time::nowFast())
{
    ...
    std::uintptr_t baseAddr = elfDynamicBaseAddress();
    std::uintptr_t baseSize = elfTextSize();
    scanningRegions.resize(kNumberOfSectionHashes);

    // set up the main hash
    scanningRegions[kGoldHashStart] = ScanRegionTest(
        ScanRegion(reinterpret_cast<char*>(baseAddr), baseSize));
    ...
    ScanRegionTest nonceLocation(
        ScanRegion(reinterpret_cast<char*>(&pmcHash.nonce), sizeof(uint32_t)));
    ...
    for (std::size_t i = 1; i < scanningRegions.size(); ++i)
    {
        scanningRegions[i] = nonceLocation;
    }
    scanningRegions.back().closeHash = true;

    std::size_t totalMemoryToScan = 0;
    for (const auto& region : scanningRegions)
    {
        totalMemoryToScan += region.size;
    }

    bytesPerStep = (totalMemoryToScan + kSteps - 1) / kSteps;

    // setup mechanism so that the currentHash is valid
...
{
#if !defined(RBX_STUDIO_BUILD) && !defined(__ANDROID__) && !defined(RBX_PLATFORM_IOS) && !ADDRESS_SANITIZER && !defined(__APPLE__)
