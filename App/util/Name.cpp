...
// FIX: guards checkRbxCaller in declare() from running during static init
// ? CRT startup call stack fails the RBX text section check on linux-release. why? no fucking clue
static rbx::atomic<bool> gStaticInitDone(false);

void Name::onStaticInitDone()
{
...
//////////////////////////////////////////
// Cumbersome code to ensure that the mutex is initialized
...
const Name &Name::declare(const char *const &sName)
{
	...
	Name *result;
	{
		...
		// FIX: Skip caller check during static init
		if (gStaticInitDone)
		{
...
const Name &Name::lookup(const char *const &sName)
{
	...
	{
		RBX::mutex::scoped_lock lock(mutex());
		// FIX: Same static init guard as declare above
		if (gStaticInitDone)
		{
