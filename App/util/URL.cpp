...
const std::string Url::domain() const
{
    size_t last_dot_pos = host_.rfind('.');
    if (last_dot_pos == std::string::npos || last_dot_pos == 0)
    {
        return host_;
    }

    size_t second_last_dot_pos = host_.rfind('.', last_dot_pos - 1);
    if (second_last_dot_pos == std::string::npos)
    {
        return host_;
    }
    
    return host_.substr(second_last_dot_pos + 1);
}
