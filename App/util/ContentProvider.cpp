...
namespace RBX {
	...
	Reflection::PropDescriptor<ContentProvider, std::string> ContentProvider::desc_baseUrl("BaseUrl", category_Data, &ContentProvider::getBaseUrl, &ContentProvider::setBaseUrl);
	...
	static Reflection::PropDescriptor<ContentProvider, int> desc_requestQueueSize("RequestQueueSize", category_Data, &ContentProvider::getRequestQueueSize, &ContentProvider::setRequestQueueSize);
    REFLECTION_END();
	...
	void ContentProvider::setAssetFolder(const char* sPath)
	{
        if (!assetFolderAlreadyInit)
		{
            if (RBX::Log::current())
				...
				assetFolderString = "/content/";
				platformAssetFolderString = "/PlatformContent/pc/";

				assetFolderPath = fs::path(assetFolderString);
				platformAssetFolderPath = fs::path(platformAssetFolderString);
			#else
