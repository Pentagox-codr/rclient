...
bool Http::isRobloxSite(const char* url)
{
    ...
    std::string baseDomain = RBX::Url::fromString(baseUrl).domain();
    if (baseDomain.empty())
    {
        return false; 
    }
    ...
    return parsed.isSubdomainOf(baseDomain.c_str());
}
...
{
    bool trustCheck = isRobloxSite(url);
    return externalRequest ? !trustCheck : trustCheck;
}
