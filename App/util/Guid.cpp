...
void RBX::Guid::generateStandardGUID(std::string &result)
{
	...
	result = "{";
	result.append(szGUID);
	result.append("}");

	boost::to_lower(result);

#else
	// Use Boost's UUID string conversion directly to avoid detail namespace
	// FIX: Moved gen to local static to avoid static init shenanigans fml
	static boost::uuids::basic_random_generator<boost::mt19937> gen;
