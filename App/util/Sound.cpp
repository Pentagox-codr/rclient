...
namespace RBX 
{
	...
	namespace Soundscape
    {
		...
		FMOD_SOUND* Sound::tryLoad(const RBX::Instance* context) // throw on failure, returns null if not available yet
		{
            ...
            try
			{
				...
				if ( !fileName.empty() && (fileSize >= DFInt::MinSoundStreamSizeBytes) )
				{
					...
					SoundService::checkResult(FMOD_System_CreateStream(system.get(), fileName.c_str(), mode, NULL, &fmod_sound), "createStream", this, system.get());
				}
				else
				{
					...
					SoundService::checkResult(FMOD_System_CreateSound(system.get(), data->c_str(), mode, &info, &fmod_sound), "createSound", this, system.get());
				}
			}
		...
		void Sound::release()
		{
			...
			if (fmod_sound)
            {
                    SoundService::checkResultNoThrow(FMOD_Sound_Release(fmod_sound), "release", this, fmod_sound);
				fmod_sound = 0;
			}
		}
	}// namespace Soundscape
