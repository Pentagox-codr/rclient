...
static Reflection::PropDescriptor<SoundChannel, double> sound_desc_SoundLength("TimeLength", category_Data, &SoundChannel::getSoundLength, &SoundChannel::setSoundLength, Reflection::PropertyDescriptor::UI);
...
static Reflection::PropDescriptor<SoundChannel, bool> sound_desc_IsPlaying("IsPlaying", category_Data, &SoundChannel::isPlaying, &SoundChannel::setIsPlaying, Reflection::PropertyDescriptor::UI);
static Reflection::PropDescriptor<SoundChannel, bool> sound_desc_isPaused("IsPaused", category_Data, &SoundChannel::isPaused, &SoundChannel::setIsPaused, Reflection::PropertyDescriptor::UI);
...
static Reflection::PropDescriptor<SoundChannel, bool> sound_desc_dep_IsPlaying("isPlaying", category_Data, &SoundChannel::isPlaying, &SoundChannel::setIsPlaying, Reflection::PropertyDescriptor::Attributes::deprecated(sound_desc_IsPlaying));
REFLECTION_END();
...
void SoundChannel::releaseChannel()
{
	...
	if (fmod_channel)
	{
		...
		SoundService::checkResultNoThrow(FMOD_Channel_GetUserData(fmod_channel, reinterpret_cast<void**>(&oldSound)), "getUserData", this, fmod_channel);
		RBXASSERT(oldSound==0 || oldSound==this);
#endif
            SoundService::checkResultNoThrow(FMOD_Channel_SetUserData(fmod_channel, reinterpret_cast<void*>(NULL)), "setUserData release", this, fmod_channel);
		fmod_channel = NULL;
	}
	if (sound)
	{
...
void SoundChannel::updateListenState(const Time::Interval& timeSinceLastStep)
{
	...
	else if (!controlledByAndIsServer() && fmod_channel && getLooped() && isPlaying())
	{
		...
		FMOD_Channel_GetPosition(fmod_channel, &currentPosMs,FMOD_TIMEUNIT_MS);
...
bool SoundChannel::isPaused() const
{
	...
	if (!invalidChannel)
	{
		FMOD_RESULT fmodResult = FMOD_Channel_GetPaused(fmod_channel, &paused);
        ...
        FASTLOG2(DFLog::SoundTrace, "Returning SoundChannel::isPaused(%p) = %d", this, paused != 0);
	}
...
bool SoundChannel::isPlaying() const
{
	...
	if (!invalidChannel)
	{
		FMOD_RESULT fmodResult = FMOD_Channel_IsPlaying(fmod_channel, &playing);
		...
		fmodResult = FMOD_Channel_GetPaused(fmod_channel, &paused);
		if (FMOD_OK != fmodResult)
		{
			invalidChannel = true;
			SoundService::checkResultNoThrow(fmodResult, "isPlaying", this, fmod_channel);
		...
		FASTLOG2(DFLog::SoundTrace, "Returning SoundChannel::isPlaying(%p) = %d", this, (playing && !paused) != 0);
	}

	return (playing && !paused) != 0;
}
...
double SoundChannel::getSoundLength() const
{
	...
	unsigned length = 0;
	FMOD_SOUND* fmod_sound = sound ? sound->tryLoad(this) : NULL;
	if (fmod_sound)
	{
		FMOD_RESULT fmodResult = FMOD_Sound_GetLength(fmod_sound, &length, FMOD_TIMEUNIT_MS);
		if (FMOD_OK != fmodResult)
		{
...
double SoundChannel::getSoundPosition() const
{
	...
	else
	{
		...
		FMOD_RESULT fmodResult = FMOD_Channel_GetPosition(fmod_channel, &positionMs, FMOD_TIMEUNIT_MS);
		if (FMOD_OK != fmodResult)
		{
			invalidChannel = true;
...
void SoundChannel::setSoundPosition(double value, bool setFromLua)
{
	...
	if (fmod_channel)
	{
		...
		if (oldPosition != newPosition)
		{
			...
			SoundService::checkResultNoThrow(FMOD_Channel_SetPosition(fmod_channel, (newPosition * 1000.0), FMOD_TIMEUNIT_MS), "setPosition", this, fmod_channel);
		}
	}
...
void SoundChannel::setVolume(float value)
{
	...
	if (volume != value)
	{
		...
		if (fmod_channel)
		{
			SoundService::checkResultNoThrow(FMOD_Channel_SetVolume(fmod_channel, value), "setVolume", this, fmod_channel);
		}
...
void SoundChannel::setPitch(float value)
{
	...
	if (pitch != value)
	{
		...
		if (fmod_channel)
		{
			SoundService::checkResultNoThrow(FMOD_Channel_SetFrequency(fmod_channel, pitch * defaultFrequency), "setFrequency", this, fmod_channel);
		}
...
void SoundChannel::updateLooped()
{
	...
	{
		FMOD_MODE mode;
		SoundService::checkResultNoThrow(FMOD_Channel_GetMode(fmod_channel, &mode), "getMode", this, fmod_channel);
		mode |= (getLooped() ? FMOD_LOOP_NORMAL : FMOD_LOOP_OFF);
		mode &= (getLooped() ? ~FMOD_LOOP_OFF : ~FMOD_LOOP_NORMAL);
		SoundService::checkResultNoThrow(FMOD_Channel_SetMode(fmod_channel, mode), "setMode", this, fmod_channel);
		SoundService::checkResultNoThrow(FMOD_Channel_SetLoopCount(fmod_channel, getLooped() ? -1 : 0), "setLoopCount", this, fmod_channel);
		...
		if (FInt::MinSecondLengthForLongSoundChannel > 0)
		{
			...
			{
				SoundService::checkResult(FMOD_Channel_SetPriority(fmod_channel, FMOD_LONG_SOUND_CHANNEL_PRIORITY), "setPriority", this, fmod_channel);
			}
		}
	}
}
...
void SoundChannel::update3D(FMOD_CHANNEL* channel)
{
	if (is3D && part)
	{
		if (DFFlag::MinMaxDistanceEnabled)
		...
		if (DFFlag::RollOffModeEnabled) 
		{
			if (rollOff == Inverse)
			{
				FMOD_Channel_SetMode(channel, FMOD_3D_INVERSEROLLOFF);
			}
		...
		if (SoundService::convert(part->getCoordinateFrame().translation, pos))
		{
			...
			{
				FMOD_RESULT fmodResult = FMOD_Channel_Set3DAttributes(channel, &pos, &vel);
...
void SoundChannel::onChannelEnd(const FMOD_CHANNEL *channel) 
{
	...
	if (fmod_channel && reinterpret_cast<const void*>(channel) == reinterpret_cast<const void*>(fmod_channel))
	{
		...
		//SoundService::checkResultNoThrow(FMOD_Channel_GetUserData(fmod_channel, reinterpret_cast<void*>(NULL)), "setUserData onChannelEnd", this, fmod_channel);
		fmod_channel = NULL;
...
FMOD_RESULT callbackChannelEnd(FMOD_CHANNELCONTROL *channelControl, FMOD_CHANNELCONTROL_TYPE type, FMOD_CHANNELCONTROL_CALLBACK_TYPE callbacktype, void *commanddata1, void *commanddata2)
{
	...
	else
	{
		...
		// FMOD_CHANNELGROUP *group = (FMOD_CHANNELGROUP *)channelControl;
	...
	if (!soundDisabled)
	{
		try
		{
			...
			if (fmod_channel && getLooped() && isPlaying())
			{
				...
				SoundService::checkResult(FMOD_Channel_SetPaused(fmod_channel, false), "setPaused play existing", this, fmod_channel);
			}
			else
			{
				...
				else
				{
					...
					else
					{
						...
						if (fmod_channel)
						{
							...
							if (isResuming)
							{
								if ( FMOD_OK == FMOD_Channel_SetPaused(fmod_channel, false) )
								{
									return;
								}
							}
							...
							{
								if (FMOD_OK == FMOD_Channel_SetPosition(fmod_channel, 0, FMOD_TIMEUNIT_MS))
								{
									setSoundPosition(0);
									if ( FMOD_OK == FMOD_Channel_SetPaused(fmod_channel, false) )
									{
						...
						sound = soundService->loadSound(getSoundId(), is3D);
						sound->acquire();
						FMOD_SOUND* fmod_sound = sound->tryLoad(context);
						...
						FMOD_RESULT fmodResult = FMOD_System_PlaySound(soundService->system.get(), fmod_sound, soundService->getMasterChannel(), true, &fmod_channel);
						if (FMOD_OK != fmodResult)
						{
						...
						SoundService::checkResult(FMOD_Sound_GetDefaults(fmod_sound, &defaultFrequency, NULL), "getDefaults play", this, fmod_sound);
						if (pitch != 1)
						{
							SoundService::checkResult(FMOD_Channel_SetFrequency(fmod_channel, pitch * defaultFrequency), "setFrequency play", this, fmod_channel);
						}

						// Wire up callbacks
						SoundService::checkResult(FMOD_Channel_SetCallback(fmod_channel, &callbackChannelEnd), "setCallback play", this, fmod_channel);

						// add to master group (for master volume control)
						FMOD_Channel_SetChannelGroup(fmod_channel, soundService->getMasterChannel());

						FASTLOG1F(DFLog::SoundTrace, "Volume is %f", volume);
						SoundService::checkResult(FMOD_Channel_SetVolume(fmod_channel, volume), "setVolume play", this, fmod_channel);
						...
						SoundService::checkResult(FMOD_Channel_SetPaused(fmod_channel, false), "setPaused play", this, fmod_channel);
					}
				}
			}
		}
...
void SoundChannel::pauseLocal()
{
	...
	if (fmod_channel)
	{
		FMOD_BOOL currentlyPaused = 0;
		FMOD_Channel_GetPaused(fmod_channel, &currentlyPaused);
		if (currentlyPaused == 0)
		{
			soundPausedSignal(getSoundId().toString());
		}

		SoundService::checkResultNoThrow(FMOD_Channel_SetPaused(fmod_channel, true), "setPaused", this, fmod_channel);
	}
}
...
void SoundChannel::stop()
{
	...
	if (fmod_channel)
	{
		...
		//SoundService::checkResultNoThrow(FMOD_Channel_GetUserData(fmod_channel, reinterpret_cast<void*>(NULL)), "setUserData", this, fmod_channel);
		SoundService::checkResultNoThrow(FMOD_Channel_Stop(fmod_channel), "stop", this, fmod_channel);
		fmod_channel = NULL;
