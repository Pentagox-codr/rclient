...
namespace RBX
{
    uint64_t Security::teaDecrypt(uint64_t inTag)
    {  
    ...
    uint64_t Security::teaEncrypt(uint64_t inTag)
    {
        Tokens::binaryTag tag;
        tag.asFull = inTag;
        uint32_t v0 = tag.asHalf[0];
    ...
    ClientFuzzySecurityToken::ClientFuzzySecurityToken(uint64_t inTag)
    {
        tag.asFull = inTag;
    }

    uint64_t ClientFuzzySecurityToken::crypt()
    {
    ...
    ServerFuzzySecurityToken::ServerFuzzySecurityToken(uint64_t inTag, unsigned int ignoreFlags)
    ...
    // easy.
    uint64_t ServerFuzzySecurityToken::decrypt(uint64_t inTag)
    {
        // Attempt to see if this is the expected next in sequence
        uint64_t decryptedValue = Security::teaDecrypt(inTag);
        uint64_t decodedValue = decryptedValue ^ lastTag.asFull;
        if (!(decodedValue >> 32))
        {
            lastTag.asFull = inTag;
            return decodedValue;
        ...
        for (int i = 0; i < (1 << popCnt); ++i)
        {
            ...
            uint64_t testTag = Security::teaEncrypt(lastTag.asFull ^ mask);
            uint64_t testDecode = (testTag ^ decryptedValue);
