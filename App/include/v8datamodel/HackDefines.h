...
template<unsigned int key> unsigned int getIndirectly(void* addr)
{
    std::uintptr_t dest = reinterpret_cast<std::uintptr_t>(addr) + key;
    volatile std::uintptr_t tmp = dest;
    return *reinterpret_cast<unsigned int*>(tmp - key);
}
}
}
