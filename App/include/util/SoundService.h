...
namespace RBX 
{
	...
	namespace Soundscape
	{
		...
		private:
			...
			rbx::event_signals::scoped_connection gameSettingsChangedConnection;
		...
		public:
			...
			shared_ptr<FMOD_SYSTEM> getSystem() const { return system; }
			SoundService();
			~SoundService();
			void playSound(SoundType sound);

			FMOD_DSP* createDSP(FMOD_DSP_DESCRIPTION &dspdesc);
			int getSampleRate();
			...
			FMOD_CHANNELGROUP* getMasterChannel() { return channelMaster; }
		...
		public:
			...
			void update()
			{
				/*if (service->enabled())
				{
					...
					FMOD_System_GetVersion(service->getSystem(), &version);
					this->formatValue(version, "fmod %08x", version);

					int current = 0, max = 0;
					if (FMOD_Memory_GetStats(service->getSystem(), &current, &max) == FMOD_OK)
					{
						currentalloced = (size_t) current;
						maxalloced = (size_t) max;
					}

					service->getSoundStats(numSounds, numUnusedSounds);

					int channels = 0;
					FMOD_System_GetChannelsPlaying(service->getSystem(), &channels);
					channelsPlaying = channels;

					float dsp = 0, stream = 0, geometry = 0, update = 0, total = 0;
					FMOD_System_GetCPUUsage(service->getSystem(), &dsp, &stream, &geometry, &update, &total);

					cpuStats.dsp = dsp;
					cpuStats.stream = stream;
					cpuStats.geometry = geometry;
					cpuStats.update = update;
