...
namespace RBX
{
	...
	static long long encodeInt64(long long value)
    {
	...
	static void readInt64Vector(MemoryInputStream& stream, std::vector<long long>& values, size_t count)
	{
		...
		if (stream.offset + count * 8 > stream.datasize)
			throw RBX::runtime_error("Read offset is out of bounds while reading %d bytes", (int)(count * 8));
		...
		for (size_t i = 0; i < count; ++i)
		{
			...
			unsigned long long raw =
				(static_cast<unsigned long long>(v0) << 56) |
				(static_cast<unsigned long long>(v1) << 48) |
				(static_cast<unsigned long long>(v2) << 40) |
				(static_cast<unsigned long long>(v3) << 32) |
				(static_cast<unsigned long long>(v4) << 24) |
				(static_cast<unsigned long long>(v5) << 16) |
				(static_cast<unsigned long long>(v6) << 8)  |
				static_cast<unsigned long long>(v7);

			values.push_back(decodeInt64(static_cast<long long>(raw)));
		}
	...
	static void writeInt64Vector(MemoryOutputStream& stream, const std::vector<long long>& values)
	{
		for (size_t i = 0; i < values.size(); ++i)
		{
			unsigned long long raw = static_cast<unsigned long long>(encodeInt64(values[i]));
			writeRaw(stream, (unsigned char)(raw >> 56));
		}

		for (size_t i = 0; i < values.size(); ++i)
		{
			unsigned char b = (unsigned char)((static_cast<unsigned long long>(encodeInt64(values[i])) >> 48) & 0xFF);
			writeRaw(stream, b);
		}

		for (size_t i = 0; i < values.size(); ++i)
		{
			unsigned char b = (unsigned char)((static_cast<unsigned long long>(encodeInt64(values[i])) >> 40) & 0xFF);
			writeRaw(stream, b);
		}

		for (size_t i = 0; i < values.size(); ++i)
		{
			unsigned char b = (unsigned char)((static_cast<unsigned long long>(encodeInt64(values[i])) >> 32) & 0xFF);
			writeRaw(stream, b);
		}

		for (size_t i = 0; i < values.size(); ++i)
		{
			unsigned char b = (unsigned char)((static_cast<unsigned long long>(encodeInt64(values[i])) >> 24) & 0xFF);
			writeRaw(stream, b);
		}

		for (size_t i = 0; i < values.size(); ++i)
		{
			unsigned char b = (unsigned char)((static_cast<unsigned long long>(encodeInt64(values[i])) >> 16) & 0xFF);
			writeRaw(stream, b);
		}

		for (size_t i = 0; i < values.size(); ++i)
		{
			unsigned char b = (unsigned char)((static_cast<unsigned long long>(encodeInt64(values[i])) >> 8) & 0xFF);
			writeRaw(stream, b);
		}

		for (size_t i = 0; i < values.size(); ++i)
		{
			unsigned char b = (unsigned char)(static_cast<unsigned long long>(encodeInt64(values[i])) & 0xFF);
			writeRaw(stream, b);
		}
	}
	...
	void readPropertyValues(MemoryInputStream& stream, const Reflection::PropertyDescriptor& desc, const std::vector<Instance*>& instances, const std::vector<shared_ptr<Instance> >& idMap)
    {
        ...
        else if (desc.type == Reflection::Type::singleton<int>())
        {
			...
			else
            {
                if (format != bpfInt)
                {
                    throw RBX::runtime_error("Unexpected format %d (expected %d)", format, bpfInt);
                }
		...
		else if (desc.type == Reflection::Type::singleton<long long>())
        {
            char format;
            readRaw(stream, format);
	...
	void writePropertyValues(MemoryOutputStream& stream, const Reflection::PropertyDescriptor& desc, const std::vector<const Instance*>& instances, const std::map<const Instance*, int>& idMap, unsigned int flags)
    {
		...
		else if (desc.type == Reflection::Type::singleton<int>())
		{
			...
			if constexpr (sizeof(int) == 8)
			{
				...
				writeInt64Vector(stream, std::vector<long long>(values.begin(), values.end()));
			}
			else
			{
		...
		else if (desc.type == Reflection::Type::singleton<long long>())
		{
			...
			for (size_t i = 0; i < instances.size(); ++i)
				values.push_back(getPropertyValue<long long>(desc, instances[i]));
