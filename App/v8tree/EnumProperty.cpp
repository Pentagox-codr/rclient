...
{
	static TType<long long> type("long long");
	return type;
}
...
template<>
const Type& Type::getSingleton<rbx::event_signals::connection>()
{
	static TType<rbx::event_signals::connection> type("Connection");
	return type;
}
...
template<>
G3D::Color3uint8& RBX::Reflection::Variant::convert<G3D::Color3uint8>(void)
{
...
template<>
void RBX::Reflection::TypedPropertyDescriptor<G3D::Color3uint8>::readValue(DescribedBase* instance, const XmlElement* element, IReferenceBinder& binder) const
{
	if (!element->isXsiNil())
	{
		unsigned int argb;
...
template<>
void RBX::Reflection::TypedPropertyDescriptor<G3D::Color3uint8>::writeValue(const DescribedBase* instance, XmlElement* element) const
{
	// Write the data out in accordance with ROBLOX Schema
