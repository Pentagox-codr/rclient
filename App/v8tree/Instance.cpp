...
static Reflection::BoundFuncDesc<Instance, shared_ptr<Instance>(std::string, bool)> findFirstChildOfClass(&Instance::findFirstChildOfClass2, "FindFirstChildOfClass", "className", "recursive", false, Security::None);
static Reflection::BoundFuncDesc<Instance, shared_ptr<Instance>(std::string, bool)> findFirstChildWhichIsA(&Instance::findFirstChildWhichIsA2, "FindFirstChildWhichIsA", "className", "recursive", false, Security::None);
static Reflection::BoundFuncDesc<Instance, shared_ptr<Instance>(std::string, bool)> findFirstChildOfClassDeprecated(&Instance::findFirstChildOfClass2, "findFirstChildOfClass", "name", "recursive", false, Security::None, Reflection::FunctionDescriptor::Attributes::deprecated(findFirstChildOfClass));
static Reflection::BoundFuncDesc<Instance, shared_ptr<Instance>(std::string, bool)> findFirstChildWhichIsADeprecated(&Instance::findFirstChildWhichIsA2, "findFirstChildWhichIsA", "name", "recursive", false, Security::None, Reflection::FunctionDescriptor::Attributes::deprecated(findFirstChildWhichIsA));
...
static Reflection::BoundFuncDesc<Instance, shared_ptr<Instance>(std::string)> func_findFirstAncestor(&Instance::findFirstAncestorByName2, "FindFirstAncestor", "name", Security::None);
static Reflection::BoundFuncDesc<Instance, shared_ptr<Instance>(std::string)> func_findFirstAncestorOfClass(&Instance::findFirstAncestorOfClass2, "FindFirstAncestorOfClass", "className", Security::None);
static Reflection::BoundFuncDesc<Instance, shared_ptr<Instance>(std::string)> func_findFirstAncestorWhichIsA(&Instance::findFirstAncestorWhichIsA2, "FindFirstAncestorWhichIsA", "className", Security::None);
...
static Reflection::PropDescriptor<Instance, std::string> prop_className("ClassName", category_Data, &Instance::getClassNameStr, &Instance::setClassNameStr, Reflection::PropertyDescriptor::UI);
static Reflection::PropDescriptor<Instance, std::string> prop_classNameDeprecated("className", category_Data, &Instance::getClassNameStr, &Instance::setClassNameStr, Reflection::PropertyDescriptor::Attributes::deprecated(prop_className));
static Reflection::PropDescriptor<Instance, int> prop_dataCost("DataCost", category_Data, &Instance::getPersistentDataCost, &Instance::setPersistentDataCost, Reflection::PropertyDescriptor::UI, Security::RobloxPlace);
...
const Reflection::RefPropDescriptor<Instance, Instance> Instance::propParent2("parent", category_Data, &Instance::getParentDangerous, &Instance::setParent, Reflection::PropertyDescriptor::LEGACY_SCRIPTING);
...
Instance* Instance::findFirstChildOfClassRecursive(const std::string& findName)
{
	// breadth-first search
	Instance* child = findFirstChildOfClass(findName);
	if (child != NULL)
		return child;
	...
	for (size_t i = 0; i < c.size(); ++i)
	{
		child = c[i]->findFirstChildOfClassRecursive(findName);
		if (child != NULL)
			return child;
	}
...
Instance* Instance::findFirstChildWhichIsARecursive(const std::string& findName)
{
	// breadth-first search
	Instance* child = findFirstChildWhichIsA(findName);
	if (child != NULL)
		return child;
	...
	for (size_t i = 0; i < c.size(); ++i)
	{
		child = c[i]->findFirstChildWhichIsARecursive(findName);
		if (child != NULL)
			return child;
	}
...
const Instance* Instance::findConstFirstChildOfClass(const std::string& findName) const
{
	if (!children)
		return NULL;
	const Instances& c(*children);
	for (size_t i = 0; i < c.size(); ++i)
	{
		if (c[i]->getClassNameStr() == findName)
		{
			return c[i].get();
		}
	}
...
const Instance* Instance::findConstFirstChildWhichIsA(const std::string& findName) const
{
	if (!children)
		return NULL;
	const Instances& c(*children);
	for (size_t i = 0; i < c.size(); ++i)
	{
		if (c[i]->getDescriptor().isA(findName.c_str()))
		{
			return c[i].get();
		}
	}
...
const Instance* Instance::findConstFirstAncestorByName(const std::string& findName) const
{
	...
	const Instance* a = getParent();
	const std::string aName = a->getName();
	if (aName != findName) return a->findFirstAncestorByName(findName);
	return a;
}

const Instance* Instance::findConstFirstAncestorOfClass(const std::string& findName) const
{
	if (!parent) return NULL;
	const Instance* a = getParent();
	const std::string aCName = a->getClassNameStr();
	if (aCName != findName) return a->findConstFirstAncestorOfClass(findName);
	return a;
}

const Instance* Instance::findConstFirstAncestorWhichIsA(const std::string& findName) const
{
	if (!parent) return NULL;
	const Instance* a = getParent();
	if (!a->getDescriptor().isA(findName.c_str())) return a->findConstFirstAncestorOfClass(findName);
	return a;
}
...
#if !defined(RBX_RCC_SECURITY) && !defined(RBX_STUDIO_BUILD) && !defined(_NOOPT) && !defined(_DEBUG) && (defined(_WIN32) || defined(linux)) && !defined(RBX_PLATFORM_DURANGO)
...
static const RBX::Name& nameParent2 = RBX::Name::declare("parent");
