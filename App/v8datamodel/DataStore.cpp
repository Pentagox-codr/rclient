...
namespace RBX {
	...
	static Reflection::BoundFuncDesc<DataStore, rbx::event_signals::connection(std::string, Lua::WeakFunctionRef)> func_onUpdate(&DataStore::onUpdate, "OnUpdate", "key", "callback", Security::None);
	...
	rbx::event_signals::connection DataStore::onUpdate(std::string key, Lua::WeakFunctionRef callback)
	{
		if (!checkAccess(key, NULL))
			return rbx::event_signals::connection();
		...
		rbx::event_signals::connection conn = signal->connect(
