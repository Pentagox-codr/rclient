...
namespace std
{
    ...
    public:
        ...
        T load(boost::memory_order order = boost::memory_order_seq_cst) const
        {
            T result = value;
            if (order != boost::memory_order_relaxed) __sync_synchronize();
            return result;
        }

        void store(T desired, boost::memory_order order = boost::memory_order_seq_cst)
        {
            if (order != boost::memory_order_relaxed) __sync_synchronize();
            value = desired;
        }

        T fetch_add(T arg, boost::memory_order order = boost::memory_order_seq_cst)
        {
