...
void Log::timeStamp(std::ofstream& stream, bool includeDate)
{
	...
	snprintf(s, ARRAYSIZE(s), "%02u:%02u:%02u.%03u (%03.07f)", (unsigned int)dur.hours(), (unsigned int)dur.minutes(), (unsigned int)dur.seconds(), (unsigned short)dur.total_milliseconds(), Time::nowFastSec());
	stream << s;
	stream.flush();
}
