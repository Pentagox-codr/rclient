...
MICROPROFILE_API boost::recursive_mutex& MicroProfileGetMutex();
...
typedef boost::thread* MicroProfileThread;
...
struct MicroProfileThreadLog
{
    ...
    boost::atomic<uint32_t>	nPut;
    boost::atomic<uint32_t>	nGet;
	uint32_t 				nActive;
	uint32_t 				nGpu;
...
struct MicroProfile
{
    ...
    boost::atomic<char*>			LabelBuffer;
    boost::atomic<uint64_t>		nLabelPut;
    ...
    boost::atomic<int64_t> 		Counters[MICROPROFILE_MAX_COUNTERS];
...
{
	*pThread = new boost::thread(Func, nullptr);
}
...
inline boost::recursive_mutex& MicroProfileMutex()
{
	static boost::recursive_mutex Mutex;
	return Mutex;
}
boost::recursive_mutex& MicroProfileGetMutex()
{
...
{
    boost::recursive_mutex& mutex = MicroProfileMutex();
...
{
    boost::lock_guard<boost::recursive_mutex> Lock(MicroProfileMutex());
...
struct MicroProfileScopeLock
{
	...
	MicroProfileScopeLock(boost::recursive_mutex& m) : bUseLock(g_bUseLock), m(m)
	{
		if(bUseLock)
			m.lock();
	}
...
inline void MicroProfileLogPut(MicroProfileToken nToken_, uint64_t nTick, uint64_t nBegin, MicroProfileThreadLog* pLog)
{
	...
	uint32_t nPos = pLog->nPut.load(boost::memory_order_relaxed);
	uint32_t nNextPos = (nPos+1) % MICROPROFILE_BUFFER_SIZE;
	if(nNextPos == pLog->nGet.load(boost::memory_order_relaxed))
	{
		S.nOverflow = 100;
	}
	else
	{
		...
		pLog->nPut.store(nNextPos, boost::memory_order_release);
	}
}
...
{
	char* pLabelBuffer = S.LabelBuffer.load(boost::memory_order_consume);
	if(!pLabelBuffer)
	{
		MicroProfileScopeLock L(MicroProfileMutex());

		pLabelBuffer = S.LabelBuffer.load(boost::memory_order_consume);
		if(!pLabelBuffer)
		{
			...
			S.LabelBuffer.store(pLabelBuffer, boost::memory_order_release);
		}
	}
	...
	uint64_t nLabel = S.nLabelPut.fetch_add(nLen + 1, boost::memory_order_relaxed);
...
{
	char* pLabelBuffer = S.LabelBuffer.load(boost::memory_order_relaxed);
	uint64_t nLabelPut = S.nLabelPut.load(boost::memory_order_relaxed);
...
void MicroProfileFlip()
{
	...
	if(S.nRunning || S.nForceEnable)
	{
		...
		for(uint32_t i = 0; i < MICROPROFILE_MAX_THREADS; ++i)
		{
			...
			{
				uint32_t nPut = pLog->nPut.load(boost::memory_order_acquire);
				...
				pLog->nGet.store(nPut, boost::memory_order_relaxed);
			}
		}
...
void MicroProfileDumpHtml(MicroProfileWriteCallback CB, void* Handle, int nMaxFrames, const char* pHost)
{
    ...
    boost_swap_impl::sort(nGroupCounterSort, nGroupCounterSort + S.nGroupCount,
	...
	boost_swap_impl::sort(nTimerCounterSort, nTimerCounterSort + S.nTotalTimers,
