...
void MicroProfileDrawDetailedBars(uint32_t nWidth, uint32_t nHeight, int nBaseY, int nSelectedFrame)
{
	...
	if(!bSkipBarView)
	{
		for(uint32_t i = 0; i < MICROPROFILE_MAX_THREADS; ++i)
		{
			...
			uint32_t nFront = S.Pool[i]->nPut.load(boost::memory_order_relaxed);
	...
	if(S.bContextSwitchRunning && (S.bContextSwitchAllThreads||S.bContextSwitchNoBars))
	{
		...
		if(S.bContextSwitchAllThreads)
		{
			...
			boost_swap_impl::sort(&nThreads[nNumThreadsBase], &nThreads[nNumThreads]);
		}
...
#define MICROPROFILE_MENU_MAX 16
	...
	if(UI.GroupMenuCount != S.nGroupCount + S.nCategoryCount)
	{
		...
		boost_swap_impl::sort(&UI.GroupMenu[0], &UI.GroupMenu[UI.GroupMenuCount],
...
void MicroProfileDraw(uint32_t nWidth, uint32_t nHeight)
{
...

	{
		...
		if(0 == once)
		{	
			boost::recursive_mutex& m = MicroProfileGetMutex();
			m.lock();
			MicroProfileInitUI();
...
{
	boost::lock_guard<boost::recursive_mutex> Lock(MicroProfileGetMutex());
