...
namespace rbx
{
	...
	*/
	...
	public:
		...
		{
			if(p_ != 0) rbx::intrusive_ptr_add_weak_ref(p_);
		}
		...
		: p_( 0 )
		{
			...
			{	
				p_ = rhs.raw();			
				rbx::intrusive_ptr_add_weak_ref(p_);
			}
		}
		...
		{
			if( p_ != 0 ) rbx::intrusive_ptr_weak_release(p_);
		}
		...
		inline void reset()
		{
			if( p_ != 0 ) 
			{
				rbx::intrusive_ptr_weak_release(p_);
				p_ = 0;
			}
		}
		...
		{
			if (p_ && rbx::intrusive_ptr_try_lock(p_))
		...
		{
			return (!p_ || rbx::intrusive_ptr_expired(p_));
		}
