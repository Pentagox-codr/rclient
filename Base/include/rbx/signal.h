...
namespace rbx
{
	...
	// in boost event_signals  
	namespace event_signals
	{
		/*
		...
		*/
		...
		private:
			...
			// TODO: Avoid contention by using one mutex per signal? Or an array of event_signals?
			...
			// SAFE_HEAP_STATIC is used instead of SAFE_STATIC to work around global variables using event_signals (like GameSettings)
			// If you use SAFE_STATIC, mutex can be destroyed before other global variables using event_signals, 
			...
			void remove(slot* item)
			{
				...
				RBXASSERT(!rbx::intrusive_ptr_expired(item));
		...
		public:
			...
			void flogPrint()
			{
				...
				boost::intrusive_ptr<typename rbx::event_signals::signal<Signature>::slot> item;
		...
		public:
			void operator()()
			{
				if (this->empty()) return;
                 ...
                 typedef typename rbx::event_signals::signal<Signature>::slot slot;
            ...
            begin:
				...
				{
					rbx::event_signals::signal<Signature>::on_error(e);
...
begin:
	...
	template<typename Signature>
	class signal : public event_signals::signal_with_args<boost::function_traits<Signature>::arity, Signature>
	{
	};
...
public:
		...
		template<typename F>
		event_signals::connection connect(const F& function)
		{
			connectionSignal();
