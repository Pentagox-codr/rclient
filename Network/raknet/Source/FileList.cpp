...
#include <io.h>
#elif !defined ( __APPLE__ ) && !defined ( __APPLE_CC__ ) && !defined ( __PPC__ ) && !defined ( __FreeBSD__ ) && !defined ( __S3E__ ) && !defined(__EMSCRIPTEN__) && !defined(ANDROID)
#include <sys/io.h>
#endif
...
void FileList::AddFile(const char *filepath, const char *filename, FileListNodeContext context)
{
	...
	{
		// If this assert hits, split up your file. You could also change BitSize_t in RakNetTypes.h to uint64_t but this is not recommended for performance reasons
