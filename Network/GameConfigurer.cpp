...
static void set_ws_url_main_thread(const char* url) {
	std::string js =
		std::string("console.log('[ws] using websocket url: ") + url + "');"
		"SOCKFS.websocketArgs = { url: '" + url + "', subprotocol: null };";
	emscripten_run_script(js.c_str());
	free((void*)url);
}

static void trampoline_set_ws_url(void* arg) {
	set_ws_url_main_thread(static_cast<const char*>(arg));
}

static void set_websocket_url(const char* url) {
	size_t len = strlen(url) + 1;
	char* heapCopy = static_cast<char*>(malloc(len));
	strcpy(heapCopy, url);

	emscripten_async_run_in_main_runtime_thread(
		EM_FUNC_SIG_VI,
		trampoline_set_ws_url,
		static_cast<void*>(heapCopy)
	);
}
#endif
...
void PlayerConfigurer::ifSeleniumThenSetCookie(const std::string& key, const std::string& value)
{
	...
	// }
	/**
	 * We don't need to set cookies for selenium tests, as we don't DO selenium tests
	 */
}
...
void PlayerConfigurer::configure(RBX::Security::Identities identity, DataModel* dm, const std::string& args, int lm, const char* vrDevice)
{
	...
	// set socket url
	#ifdef EMSCRIPTEN
		set_websocket_url(getParamString("WebSocketAddress").c_str());
	#endif
	...
	try
	{
		...
		// }
		// We do NOT need this shit thx ❤️

		// player->setSuperSafeChat(getParamBool("SuperSafeChat"));
		// player->setUnder13(getParamBool("IsUnknownOrUnder13"));

		player->setSuperSafeChat(false); // lowkey fuck SuperSafeChat
		player->setUnder13(false); // users MUST be 13+ to play on projectpizza.net, and that is final
