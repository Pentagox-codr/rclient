...
{
    const unsigned int mask = 31; // for 32-bit values
    count &= mask;
...
#define LE(p) (le32(p))
    uint32_t in[16] = {
        ...
        LE(key + 12),        LE(c + 4),      static_cast<uint32_t>(nonce), static_cast<uint32_t>(nonce >> 32),
        static_cast<uint32_t>(index), static_cast<uint32_t>(index >> 32), LE(c + 8), LE(key + 16),
