    ...
    namespace Network
    {
...
#if defined(_WIN32) && !defined(RBX_STUDIO_BUILD) && !defined(RBX_PLATFORM_DURANGO) && false
        ...
        class ClientReplicator::BadAppCheckerJob : public DataModelJob
        {
            ...
            void doScans(unsigned int scanMask)
            {
                ...
                isCeDetectedLocal = isCeDetectedLocal || (ceHwndChecks && (scanMask & kCeTitleScanMask) && hwndScanner.detectTitle());
                isCeDetectedLocal = isCeDetectedLocal || (ceHwndChecks && (scanMask & kCeAttachScanMask) && hwndScanner.detectFakeAttach());
                isCeDetectedLocal = isCeDetectedLocal || (ceHwndChecks && (scanMask * kCeAttachScanMask) && hwndScanner.detectEarlyKick());
#if 0
        ...
        public:
            BadAppCheckerJob(shared_ptr<ClientReplicator> clientReplicator) : DataModelJob("BatteryProfiler", DataModelJob::None, false,
        ...
        public:
            ...
            MemoryCheckerJob(shared_ptr<ClientReplicator> clientReplicator) : DataModelJob("US14116", DataModelJob::None, false,
                                                                                           shared_from(DataModel::get(clientReplicator.get())), Time::Interval(0)),
                                                                              clientReplicator(clientReplicator),
                                                                              lastRunTime(Time::nowFast()),
            ...
            virtual TaskScheduler::StepResult stepDataModelJob(const Stats &stats)
            {
...
#ifdef _WIN32
                // firstHash, initialProgramHash, and lastCompletedHash should all be equa
                int diff = (firstHash != initialProgramHash) | (memoryChecker->getLastCompletedHash() != initialProgramHash);
#else
                int diff = 0;
#endif
                ...
                {
                    hashReadySignal();
#if defined(_WIN32) && !defined(RBX_PLATFORM_DURNAGO) && false
...
#if defined(_WIN32) && !defined(RBX_PLATFORM_DURANGO) && !defined(RBX_STUDIO_BUILD) && false
        // Periodically check that the program memory hashing job is still running
        ...
        public:
            ...
            MemoryCheckerCheckerJob(shared_ptr<ClientReplicator> clientReplicator) : DataModelJob("US14116_pt2", DataModelJob::None, false,
                                                                                                  shared_from(DataModel::get(clientReplicator.get())), Time::Interval(0)),
                                                                                     clientReplicator(clientReplicator),
                                                                                     runSlips(0),
                                                                                     hashSlips(0),
            ...
            virtual TaskScheduler::StepResult stepDataModelJob(const Stats &stats)
            {
                ...
                if (vehHookLocationHv)
                {
                    ...
                    isVehUnhook = ((hookLoc + sizeof(void *) + relJump) != jumpTarget) || (!ntApi.isNtdllAddress(reinterpret_cast<uintptr_t>(vehHookContinue))); // check for basic hook
                }
                if (runCount % 8 == 7)
                {
...
ClientReplicator::ClientReplicator(RakNet::SystemAddress systemAddress, Client *client, RakNet::SystemAddress clientAddress, NetworkSettings *networkSettings)
    : Super(systemAddress, client->rakPeer, networkSettings, /*ClusterDebounce*/ true), clientAddress(clientAddress), receivedGlobals(false), numInstancesRead(0), pendingInstanceRequests(0), loggedLowMemWarning(false), avgInstancesPerStreamData(0.05, 25) // estimated default value 25
      ,
      avgStreamDataReadTime(0.1, 0.05), avgRequestCount(0.1), clientInstanceQuota(0), sampleTimer(-1.0f) // guarantee fire on first call
      ,
...
#if defined(_WIN32) || (defined(__APPLE__) || defined(linux) && !defined(RBX_PLATFORM_IOS))
...
#if !defined(LOVE_ALL_ACCESS) && defined(_WIN32) && !defined(RBX_PLATFORM_DURANGO) && false
        badAppCheckerJob.reset(new BadAppCheckerJob(shared_from(this)));
        TaskScheduler::singleton().add(badAppCheckerJob);
#endif

#if !defined(LOVE_ALL_ACCESS) && (defined(_WIN32) || (defined(__APPLE__) || defined(linux) && !defined(RBX_PLATFORM_IOS))) && !defined(RBX_PLATFORM_DURANGO)
...
{
    uintptr_t thisTag = Tokens::apiToken.crypt();
    uintptr_t prevTag = Tokens::apiToken.getPrev();
...
void ClientReplicator::streamOutTerrain(const Vector3int16 &cellPos)
{
    ...
    {
        if (Voxel::Grid *grid = megaClusterInstance->getVoxelGrid())
            grid->setCell(cellPos, Voxel::Constants::kUniqueEmptyCellRepresentation, Voxel::CELL_MATERIAL_Unspecified);
    }
}
    ...
    if (classIter != serverClasses.end())
    {
        ...
        if (reflectionClass->needSync == true)
        {
            ...
            for (; propIter != reflectionClass->properties.end(); propIter++)
            {
                ...
                if (reflectionProperty->needSync == true)
                {
                    if ((cacheType == PropertyCacheType_All || isPropertyCacheable(reflectionProperty->type, reflectionProperty->enumMSB > 0) == cacheable) && (reflectionProperty->name != Instance::propParent.name)) // don't read parent for now
                    {
                ...
                else
                {
                    ...
                    while (iter != end)
                    {
                        ...
                        {
                            if (descriptor.canReplicate() && (cacheType == PropertyCacheType_All || isPropertyCacheable(descriptor.type) == cacheable) && (!cacheable || descriptor != Instance::propParent)) // don't read parent if it's cacheable (PropertyCacheType_Cacheable) or potentially cacheable (PropertyCacheType_All)
                            {
                                if (valueArray)
                                {
