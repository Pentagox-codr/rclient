...
void CheatHandlingServerReplicator::preauthenticatePlayer(int userId)
{
	try
	{
		if (server->getIsPlayerAuthenticationRequired())
		{
			...
			{
				// verify now!
				Crypt().verifySignatureHex(message, signature2);
			}
...
void CheatHandlingServerReplicator::installRemotePlayer(const std::string& preferedSpawnName)
{
	...
	if (server->getIsPlayerAuthenticationRequired())
	{
		...
		{
			// verify now!
			Crypt().verifySignatureHex(message, signature);
		}
		catch(RBX::base_exception&)
		{
...
#if defined(RBX_RCC_SECURITY)
void CheatHandlingServerReplicator::decodeHashItem(PmcHashContainer& netHashes, uint64_t* securityTokens)
{
    using namespace RBX::Hasher;
...
void CheatHandlingServerReplicator::processHashValuePost(const uint64_t* const tokens, unsigned int nonce)
{
    const uint64_t& clientToken = tokens[0];
    const uint64_t& rxApiToken = tokens[1];
    const uint64_t& rxPrevApiToken = tokens[2];
    uint64_t hackFlags = securityToken.decrypt(clientToken); // updates securityToken
    if (hashInitialized)
    {
...
#if defined(RBX_RCC_SECURITY)
void CheatHandlingServerReplicator::processApiStats(uint64_t apiStats)
{
...
#if defined(RBX_RCC_SECURITY)
void CheatHandlingServerReplicator::updateHashState(PmcHashContainer& netHashes, uint64_t* securityTokens)
{
    size_t numItems = netHashes.hash.size();
    if (!hashInitialized)
    {
...
void CheatHandlingServerReplicator::PlaceAuthenticationThreadImpl(int previousPlaceId, int requestedPlaceId)
{
    ...
    char urlBuf[2048] = {0};
	sprintf_s(urlBuf, 2048, "%suniverses/validate-place-join?originPlaceId=%d&destinationPlaceId=%d", ::GetBaseURL().c_str(), previousPlaceId, requestedPlaceId);
...
#ifdef NETWORK_DEBUG
        StandardOut::singleton()->printf(MESSAGE_INFO, "Place authentication requested (%d -> %d). Url: %s, Result: %s", previousPlaceId, requestedPlaceId, url.c_str(), response.c_str());
#endif
