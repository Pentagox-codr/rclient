	...
	while (true)
	{
		...
		while (gThumbRenderQueue.pop_if_present(request))
		{
			...
			try
			{
                ...
                // filetype==OBJ
                if (strcmp(request.fileType.c_str(), "obj") == 0)
                {
                    request.generator->exportScene(dummyView.get(), request.strOutput);
                }
				else
				{
					request.generator->renderThumb(dummyView.get(), request.fileType, request.cx, request.cy, request.hideSky, request.crop, request.strOutput);
				}
...
volatile std::atomic<long> ThumbnailGenerator::totalCount = 0;
