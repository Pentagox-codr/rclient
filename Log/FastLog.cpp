...
#pragma warning(disable: 4996)
		...
		if (static_cast<int>(reinterpret_cast<std::intptr_t>(arg0)) == FLOAT_MARKER)
        ...
        void writeWorker()
        {
            ...
            while (true)
            {
                ...
                if (entry)
                {
                    ...
                    // append entry data
                    header[1] = static_cast<int>(history);
        ...
        const char* message, ... )
    {
        ...
        if (level > LOGCHANNELS)
        {
            if(logF)
            {
                ...
                printMessageFormatted(temp2, MAX_LOG_MESSAGE, (unsigned)pthread_self(), (float)timeF(), temp);
        ...
        entry.threadid = (unsigned)pthread_self();
		...
		const void* arg4)
	{
		if (level > LOGCHANNELS)
		{
			if(logF)
			{
				...
				printMessage(temp, MAX_LOG_MESSAGE, pthread_self(), (float)timeF(), message, arg0, arg1, arg2, arg3, arg4);
				logF(level - LOGCHANNELS, temp);
				return;
			}
			else
	...
	void FastLogS(Channel level, const char* message, const char* sarg, int sargsize)
	{
		if (level > LOGCHANNELS)
		{
			if(logF)
			{
				char temp[MAX_LOG_MESSAGE];
				memset(temp, 0, sizeof(temp));
				printMessage(temp, MAX_LOG_MESSAGE, pthread_self(), (float)timeF(), message, sarg);
				logF(level - LOGCHANNELS, temp);
				return;
			}
			else
