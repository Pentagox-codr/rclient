...
public:	
	...
	: RBX::DataModelJob("Render", RBX::DataModelJob::Render, false, pDataModel, RBX::Time::Interval(0))
	, m_pDataModel(pDataModel)
	, m_bIsAwake(false)
	, updateView(false)
	, viewUpdated(false)
...
{
	m_pQtWrapperWidget->setRobloxView(NULL);
	{
...
void RobloxView::getCursorPos(G3D::Vector2* pPos)
{
    ...
    QPointF localPosF = m_pQtWrapperWidget->mapFromGlobal(globalPos);
...
bool RobloxView::isValidCursorPos(G3D::Vector2 *pPos, bool isLMB, bool force)
{
    ...
    {         
        ...
        bool insideWindow = m_pQtWrapperWidget->geometry().contains(globalCursorPos);
        return !insideWindow;
    }
...
{
    m_pQtWrapperWidget->requestActivate();
...
static RBX::ViewBase* createGameWindow(QOgreWidget *pQtWrapperWindow)
{
	...
	context.hWnd   = reinterpret_cast<void*>(pQtWrapperWindow);
	context.width  = 800;
	context.height = 600;

	RBX::CRenderSettings& renderSettings   = CRenderSettingsItem::singleton();			
	pRbxView = RBX::ViewBase::CreateView(RBX::CRenderSettings::OpenGL, &context, &renderSettings);
