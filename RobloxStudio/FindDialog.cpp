...
bool BasicFindDialog::getSearchParameters(bool forward, QRegularExpression& outRegExp, QTextDocument::FindFlags& outflags)
{
    ...
    QString pattern = toSearch;
    if (!m_pRegExCheckBox->isChecked())
        pattern = QRegularExpression::escape(toSearch);

    outRegExp.setPattern(pattern);
    outRegExp.setPatternOptions(m_pCaseCheckBox->isChecked() ? QRegularExpression::NoPatternOption : QRegularExpression::CaseInsensitiveOption);
...
bool ReplaceDialog::isSelectionValid()
{
    ...
    if (!selectedText.isEmpty())
    {
        ...
        if (!toSearch.isEmpty())
        {
            ...
            if (reg.match(selectedText).hasMatch())
                return true;
        }
    }
...
{
    QString pattern = m_pFindComboBox->lineEdit()->text();
