...
const CategoryWebSettings kWebSettings[ENTITY_CATEGORY_MAX] =
{
	// Places
	{
		...
		"%WEB_BASE%/places/create?universeId=%UNIVERSE_ID%",
		"%WEB_BASE%/places/%ITEM_ID%/updatesettings",
...
{
	qRegisterMetaType<boost::function<void(QPoint const&)>>(
		QMetaObject::normalizedType("boost::function<void(QPoint const&)>"));
    return QVariant::fromValue(val);
}
...
{
	return properties.get<int>("RootPlace").has_value();
}
...
static void addNewPlaceHelperThread(int universeId, bool* succeeded)
{
	...
	std::string defaultSettingsUrl = formatUrl(
		"%WEB_BASE%/places/defaultsettings?universeId=%UNIVERSE_ID%&templateId=%ITEM_ID%",
