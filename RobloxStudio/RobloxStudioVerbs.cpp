...
void PublishToRobloxAsVerb::initDialog()
{
	...
	try
	{
		QString initialUrl = QString("%1/ide/publishas").arg(RobloxSettings::getWebURL());
...
void PublishToRobloxAsVerb::doIt(RBX::IDataState*)
{
	...
	{
		// "http://www.roblox.com/ide/publish"
		QString initialUrl = QString("%1/ide/publish").arg(RobloxSettings::getWebURL());
...
void PublishSelectionToRobloxVerb::doIt(RBX::IDataState*)
{
	...
	QString initialUrl;
	if(isScript)
		initialUrl = QString("%1/UI/Save.aspx?type=Lua").arg(RobloxSettings::getWebURL());
	else if (isAnimation)
	{
		initialUrl = QString("%1/studio/animations/publish").arg(RobloxSettings::getWebURL());
		...
		initialUrl = QString("%1/UI/Save.aspx?type=Model").arg(RobloxSettings::getWebURL());
...
void PlaySoloVerb::doIt(RBX::IDataState*)
{
	...
	QString script;
	script = QString("loadfile(\"%1/game/visit?IsPlaySolo=1&placeId=%2&universeId=%3\")()\n")
...
void OpenToolBoxWithOptionsVerb::doIt(RBX::IDataState* dataState)
{
	...
	if (!setUrl.isEmpty())
	{
		...
		robloxToolBox.loadUrl(QString("%1/%2").arg(RobloxSettings::getWebURL()).arg(setUrl));
