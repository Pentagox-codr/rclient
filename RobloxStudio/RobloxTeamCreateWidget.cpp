...
: m_GroupId(groupId)
{
	...
	RBX::HttpAsync::get(str.toStdString());
	boost::bind(&invokeGroupDataLoaded, QPointer<GroupItem>(this), _1);
}
...
void GroupItem::groupDataLoaded(RBX::HttpFuture future)
{
	...
	if (thumnailUri.is_initialized() && !thumnailUri.get().empty())
		RBX::HttpAsync::get(thumnailUri.get());
	boost::bind(&invokeGroupThumbnailLoaded, QPointer<GroupItem>(this), _1);
}
...
void RobloxTeamCreateWidget::onPlaceIdChanged(const RBX::Reflection::PropertyDescriptor* desc)
{
	...
	//m_PlaceManageStatusQuery = RBX::HttpAsync::get(canManageUrl.toStdString());
	boost::bind(&RobloxTeamCreateWidget::updatePlaceManageStatus, this, _1);
}
...
void RobloxTeamCreateWidget::updatePlaceManageStatus(RBX::HttpFuture future)
{
	...
	//m_PlaceDetailsQuery = RBX::HttpAsync::get(url.toStdString());
	boost::bind(&RobloxTeamCreateWidget::updatePlaceDetails, this, _1);
}
...
void FriendsInfoLoader::loadPage(int pageToLoad)
{
	...
	RBX::HttpAsync::get(str.toStdString());
	boost::bind(&invokeFriendsDataLoaded, QPointer<FriendsInfoLoader>(this), pageToLoad, _1);
}
...
void CreatorsListLoader::loadPage(int pageToLoad)
{
	...
	RBX::HttpAsync::get(str.toStdString());
	boost::bind(&invokeCreatorsListLoaded, QPointer<CreatorsListLoader>(this), pageToLoad, _1);
}
