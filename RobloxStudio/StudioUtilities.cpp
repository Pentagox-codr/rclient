...
namespace StudioUtilities
{
	...
	{
		return url.contains("join", Qt::CaseInsensitive);
	}

	bool containsVisitScript(const QString& url)
	{
		return url.contains("visit", Qt::CaseInsensitive);
	}
		...
		std::string dataString;
		{
			try 
			{
				...
				std::unique_ptr<std::istream> stream = 
					RBX::ServiceProvider::create<RBX::ContentProvider>(pDataModel)->getContent(RBX::ContentId(urlScript.c_str()));
