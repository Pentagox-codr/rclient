...
bool RobloxIDEDoc::openStream(const QString& fileName, std::istream* stream, bool asNew)
{
	...
	try 
	{
		...
		if (graphicsMode != RBX::CRenderSettings::NoGraphics)
		{
			...
			QWidget* ogreContainer = QWidget::createWindowContainer(m_pQOgreWidget, m_WrapperWidget);

            if (m_pMainWindow->getBuildMode() == BM_BASIC)
                m_WrapperWidget->setMinimumSize(QSize(800, 600));
            else
                m_WrapperWidget->setMinimumSize(ogreContainer->minimumSize());
...
void RobloxIDEDoc::onIdeRun(bool play)
{
	...
	QString pathStr = QString("game/visit?isPlaySolo=1&userId=%1&placeId=%2&universeId=%3")
	...
	QString scriptStr = QString("loadfile(\"%1\")()\n")
...
{
	return m_PlayGame.m_Game != nullptr;
	//return true;
}
		...
		if (Instance* defaultToolboxSearchInstance = m_EditGame.m_Game->getDataModel()->findFirstChildByName("DefaultToolboxSearch"))
			if (StringValue* stringValue = RBX::Instance::fastDynamicCast<StringValue>(defaultToolboxSearchInstance))
			{
				QMetaObject::invokeMethod(&UpdateUIManager::Instance().getViewWidget<RobloxToolBox>(eDW_TOOLBOX),
					...
					QString("%1/%2%3").arg(RobloxSettings::getWebURL()).arg("ide/clienttoolbox?searchset=").arg(stringValue->getValue().c_str())));
			}
}
...
{	
	...
	QVariant resumeFunctionVar = QVariant::fromValue(resumeFunction);
	QVariant errorFunctionVar = QVariant::fromValue(errorFunction);
...
void RobloxIDEDoc::showImportAssetDialog(QVariant resumeFunctionVar, QVariant errorFunctionVar)
{
	...
	QString importUrl(QString("%1/studio/animations/import").arg(RobloxSettings::getWebURL()));	
...
QString RobloxIDEDoc::getRecentSaveFile()
{
    ...
    if (QDir().mkpath(recentSavesPath))
    {
        ...
        QRegularExpression invalidCharsPattern("[" + QRegularExpression::escape("\\/:*?\"<>|") + "]");
        baseName.replace(invalidCharsPattern, "_");
