...
#endif
		RBX::Analytics::setLocation("None");
...
void RobloxMainWindow::setupLogging()
	...
	RBX::Log::setLogProvider(provider);
}
...
void RobloxMainWindow::toggleFullScreen(bool state)
{
	...
	if (isRibbonStyle())
	{
        RibbonPage* pPage = ribbonBar()->getPage(ribbonBar()->currentIndexPage());
        if (pPage)
        {
...
void RobloxMainWindow::openStartPage(bool checked, QString optionalQueryParams /* = "" */)
{
	if(checked)
	{
		...
		if (startPageUrl && startPageUrl[0] != '\0')
		{
			...
			fileToOpen = (RobloxSettings::getWebURL()).append(startPageUrl).append("?");
			RobloxSettings settings;
			...
			fileToOpen = RobloxSettings::getWebURL() + "/My/Places.aspx";
...
bool RobloxMainWindow::filePublishedProjects()
{
	...
	WebDialog dlg(
        this, 
        url.prepend(RobloxSettings::getWebURL()),
        NULL, 
...
void RobloxMainWindow::publishGame()
{
	...
	QString initialUrl = QString("%1/ide/publishgameas").arg(RobloxSettings::getWebURL());
...
void RobloxMainWindow::about()
{
	...
	copyrightString.asprintf("are trademarks of ROBLOX Corporation %c2014. ", COPYRIGHT_PREFIX);
...
void RobloxMainWindow::managePlugins()
{
	...
	m_managePluginsDoc->open(this,
		QString("%1/studio/plugins/manage").arg(RobloxSettings::getWebURL()));
...
void RobloxMainWindow::closeEvent(QCloseEvent* evt)
{
	...
	RobloxNetworkAccessManager::DestroyInstance();
...
bool RobloxMainWindow::handleFileOpen(const QString &fileName, IRobloxDoc::RBXDocType type, const QString &script)
{
    ...
    {
        RBX::RobloxGoogleAnalytics::trackEvent(GA_CATEGORY_STUDIO, "EditMode", script.contains("visit") ? "Build" : "Edit");
    }
...
void RobloxMainWindow::cloudEditStatusChanged(int placeId, int universeId)
{
	...
	if (FFlag::UseBuildGenericGameUrl)
	{
		...
		initScript = QString::fromStdString(BuildGenericGameUrl(RobloxSettings::getWebURL().toStdString(), pathStr.toStdString()));
	}
	else
	{
		initScript = QString("%1/Game/edit.ashx?PlaceID=%2&upload=%2&testmode=false&universeId=%3").arg(RobloxSettings::getWebURL()).arg(placeId).arg(universeId);
	}
...
{
    m_lastAnalyticTrackedTime.start();
}

void RobloxMainWindow::keepAliveAnalyticsSession()
{
    m_lastAnalyticKeepAliveTime.start();
}
