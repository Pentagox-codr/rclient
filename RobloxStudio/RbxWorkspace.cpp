...
bool RbxWorkspace::SaveUrl(const QString &saveUrl)
{
	...
	try 
	{	
		std::string urlStr = RobloxSettings::getBaseURL().toLatin1().toStdString() + saveUrl.toStdString();
...
void RbxWorkspace::loadContent(RBX::ContentId contentId, RBX::Instances& instances, bool& hasError)
{
	try
	{
		std::unique_ptr<std::istream> stream(
			RBX::ServiceProvider::create<RBX::ContentProvider>(m_pDataModel)->getContent(contentId)
		);
		Serializer().loadInstances(*stream, instances);
		hasError = false;
	}
