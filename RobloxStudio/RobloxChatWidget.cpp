...
void ChatMessageItem::paint(QPainter* painter, const QStyleOptionViewItem& option)
{
	...
	int messageTextWidth = QFontMetrics(option.font).horizontalAdvance(messageText) + 2;
	int subLabelTextWidth = (!sublabelText.isEmpty() ? QFontMetrics(subLabelFont).horizontalAdvance(sublabelText) : 0) + 
