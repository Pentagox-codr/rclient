...
namespace Studio {
		...
		const boost::optional<std::vector<RBX::ScriptAnalyzer::IntellesenseResult> >& result)
    {
        ...
        else if (key >= 0 && key <= 0xFFFF && QChar(key).isLetter() || 
    ...
    bool Intellesense::replaceTextWithCurrentItem(QString& text, int& cursorIndex)
    {
        ...
        QString previousText = text.left(text.lastIndexOf(QRegularExpression("[\\.\\:]"), offset) + 1);
    ...
    int Intellesense::populateItemLists(QString text, int row, int index, bool searchDatamodel, const boost::optional<std::vector<RBX::ScriptAnalyzer::IntellesenseResult>>& result)
    {
        ...
        QRegularExpression service(FFlag::StudioIntellisenseCodeExecutionFixEnabled ?
            QStringLiteral("([gG]ame:(?:Find|Get)Service\\([^)]+\\)[.a-zA-Z\\d]*)([.:][a-zA-Z\\d]*)$") :
            QStringLiteral("([gG]ame:(?:Find|Get)Service\\(.+\\))[.:][a-zA-Z\\d]*$"));
        ...
        // Getting last intellisense triggering character
        // Note: lastIndexOf returns -1 if not found; use qMax to avoid negative index
        int lastIndex = text.lastIndexOf(QRegularExpression("[^a-zA-Z\\d\\_]"), index - text.size() - 1);
        int safeIndex = qMax(lastIndex, 0);
        QChar previousDivider = text.at(safeIndex);
        ...
        if (previousDivider == '.') {
            if (repopulateItemMetadata(previousWord, true, QStringLiteral("LuaLibrary")) || repopulateItemMetadata(previousWord, true))
        ...
        boost::shared_ptr<RBX::LuaSourceContainer> script;
        if (!searchDatamodel && previousWord.startsWith(QStringLiteral("script"), Qt::CaseSensitive)) {
            script = getCurrentScript();
            searchDatamodel = true;
        }

        if (previousWord.startsWith(QStringLiteral("game"), Qt::CaseInsensitive) || previousWord.startsWith(QStringLiteral("workspace"), Qt::CaseInsensitive))
            searchDatamodel = true;
    ...
    void IntellesenseTooltip::showText(const QPoint& pos, const QString& text, QWidget* focusWidget, const QPoint& bottomLeftAlt, const QPoint& topLeftAlt)//, int altX, const QPoint& altPos)
    {
        ...
        QRect screenBounds;
        if (focusWidget)
            screenBounds = QApplication::primaryScreen()->geometry();
        else
            screenBounds = QApplication::primaryScreen()->geometry();
