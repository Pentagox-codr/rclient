...
void UpdateUIManager::initDocks()
{
	...
	for ( int i = 0 ; i < eDW_MAX ; ++i )
	{
		...
		QObject::disconnect(m_DockWidgets[id]->toggleViewAction(), &QAction::toggled, m_DockWidgets[id], &QDockWidget::setVisible);
...
{		
	...
	m_pTaskSchedulerSBLabel->setContentsMargins(5,5,5,5);
	m_pTimeSBLabel->setContentsMargins(5,5,5,5);
	m_pCPUSBLabel->setContentsMargins(5,5,5,5);
	m_pRAMSBLabel->setContentsMargins(5,5,5,5);
	m_pFPSSBLabel->setContentsMargins(5,5,5,5);
...
bool UpdateUIManager::eventFilter(QObject* watched, QEvent* evt)
{
    ...
    if((evt->type() == QEvent::Show || evt->type() == QEvent::Hide) && watched->inherits("QDockWidget"))
	{
		...
		{
			EDockWindowID id = (EDockWindowID)i;
			if (m_DockWidgets[id] && !m_DockWidgets[id].isNull() && m_DockWidgets[id]->objectName() == dockName)
