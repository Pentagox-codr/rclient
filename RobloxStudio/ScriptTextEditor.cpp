...
{
    QTextCursor cursor = textCursor();
    QString foldEnd = QStringLiteral("end");

    QRegularExpression commentMatchingExp(R"((--)?\[=*\[)");

    if (text.contains(QRegularExpression(R"(\brepeat\b)")) && 
        !text.contains(QRegularExpression(R"(\buntil\b)")))
    {
        foldEnd = QStringLiteral("until");
    }
    else
    {
        ...
        if (match.hasMatch())
        {
            QString opening = match.captured(0);
            QString closing = opening;
            closing.replace('[', ']');
    ...
    QTextCursor& cursor)
{
    ...
    while (offset >= 0 && offset <= text.length())
    {
        ...
        {
            QRegularExpressionMatchIterator it = expr.globalMatch(text.left(offset + 1));
            ...
            while (it.hasNext())
            {
                ...
                if (m.hasMatch()) {
                    lastMatch = m;
                    lastMatchStart = m.capturedStart();
                }
            }
    ...
    cursor.setPosition(block.position() + idx + matchLen, QTextCursor::KeepAnchor);
    return true;
}
    ...
    int& end)
{
    ...
    while (offset >= 0 && offset <= text.length())
    {
        ...
        if (options & QTextDocument::FindWholeWords)
        {
            ...
            const int endIdx = startIdx + matchLen;

            if ((startIdx != 0 && ScriptTextEditor::isWordCharacter(text.at(startIdx - 1))) ||
                (endIdx < text.length() && ScriptTextEditor::isWordCharacter(text.at(endIdx))))
            {
                offset = endIdx + 1;
                idx = -1;
                continue;
