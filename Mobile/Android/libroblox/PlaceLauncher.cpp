...
static RBX::ProtectedString fetchAndValidateScript(RBX::DataModel* dataModel, const std::string& urlScript)
{
	...
	if (RBX::ContentProvider::isUrl(urlScript))
	{
		...
		std::unique_ptr<std::istream> stream(RBX::ServiceProvider::create<RBX::ContentProvider>(dataModel)->getContent(RBX::ContentId(urlScript.c_str())));
...
{
    StandardOut::singleton()->printf(MESSAGE_INFO, "gayyyyy3");
    ...
    StandardOut::singleton()->printf(MESSAGE_INFO, "gayyyyy4");
    initClientSettings();
    StandardOut::singleton()->printf(MESSAGE_INFO, "gayyyyy5");
...
{
    StandardOut::singleton()->printf(MESSAGE_INFO, "gayyyyy1");
	gameParams = sgp;
    ...
    {
        StandardOut::singleton()->printf(MESSAGE_INFO, "gayyyyy2");
    	currentGame = game;
