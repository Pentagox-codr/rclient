...
// This job calls ViewBase::render(), which needs to be done exclusive to the RBX::DataModel.
// This is why it has the RBX::DataModelJob::Render enum, which prevents concurrent writes to RBX::DataModel.
...
public:
	...
	static void scheduleRender(weak_ptr<RenderJob> selfWeak, RBX::ViewBase* view, double timeJobStart)
	{
...
void RobloxView::replaceGame(shared_ptr<RBX::Game> game)
{
    ...
    if(RBX::DataModel* dm = game->getDataModel().get())
    {
        if(RBX::UserInputService* inputService = RBX::ServiceProvider::create<RBX::UserInputService>(dm))
        {
            ...
            inputService->textBoxGainFocus.connect( boost::bind(&RBX::JNI::textBoxFocused, _1) );
            inputService->motionEventListeningStarted.connect( boost::bind(&RBX::JNI::motionEventListening, _1) );
            inputService->textBoxReleaseFocus.connect( boost::bind(&RBX::JNI::textBoxFocusLost, _1) );
        }
...
//
//	if(placeIDChanged && RBX::DataModel->getPlaceID() > 0)
//		Roblox::addBreakPadKeyValue("Place0", RBX::DataModel->getPlaceID());
#endif
}
