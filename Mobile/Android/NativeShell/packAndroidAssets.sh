...
# Parameters
OBB_PACKAGE_NAME="com.projectpizza.client"
OBB_VERSION="1"
...
for folder in "${INCLUDE_DIRS[@]}" ; do
    ...
    COMPARE_RESULTS=$(diff -arq $APP_DIR/../../.assets/$FROM $OUTPUT_FOLDER/$TO -x .DS_Store 2>/dev/null)
...
if [ $LOCAL_UP_TO_DATE -eq 1 ]; then
    ...
    for folder in "${INCLUDE_DIRS[@]}" ; do
        ...
        cp -R $APP_DIR/../../.assets/$FROM $OUTPUT_FOLDER/$TO
    done
